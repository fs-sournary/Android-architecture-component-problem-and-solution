 ...
 * Problem 7: Use repository as a suggestion to bridge
 */
@FlowPreview
...
class MovieRepository(private val movieDbApi: MovieDbApi, private val genreDao: GenreDao) {
    ...
    fun getMovies(genre: Genre): LiveData<PagingData<Movie>> = Pager(
    ...
    fun getRatedMovies(movieId: Int): LiveData<PagingData<Movie>> = Pager(
        config = PagingConfig(pageSize = NETWORK_PAGE_SIZE),
        pagingSourceFactory = {
